...
import com.example.Tracker.api.factory.TaskStateFactory;
import com.example.Tracker.storage.entity.ProjectEntity;
import com.example.Tracker.storage.entity.TaskStateEntity;
import com.example.Tracker.storage.repository.TaskStateRepository;
...
public class TaskController {
    ...
    TaskStateFactory taskStateDtoFactory;
    ...
    public TaskStateDto createTaskSate(
        ...
        if (taskStateName.trim().isEmpty()) {
            throw new RequestException("Task state name can't be empty.");
        }
        ...
        for (TaskStateEntity taskState: project.getTaskStates()) {
            ...
            if (taskState.getName().equalsIgnoreCase(taskStateName)) {
                throw new RequestException(String.format("Task state \"%s\" already exists.", taskStateName));
            }
        ...
        Optional<TaskStateEntity> optionalNewLeftTaskState = optionalLeftTaskStateId
                .map(leftTaskStateId -> {
                    ...
                    if (taskStateId.equals(leftTaskStateId)) {
                        throw new RequestException("Left task state id equals changed task state.");
                    }
                    ...
                    if (!project.getId().equals(leftTaskStateEntity.getProject().getId())) {
                        throw new RequestException("Task state position can be changed within the same project.");
                    }
    ...
    public AnsDto deleteTaskState(@PathVariable(name = "task_state_id") Long taskStateId) {
